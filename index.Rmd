...
p1 <- print(p + geom_smooth(method = "lm", se = FALSE) + xlab("Mean deviation (MD)") + ylab("Difference in mean deviation (dB)") + ggtitle("Bland-Altman plots (HFA-Retinalogik)"))

p <- bland.altman.plot(mdpsd$psd_hfa, mdpsd$psd_retinalogik, graph.sys = "ggplot2")

p2 <- print(p + geom_smooth(method = "lm", se = FALSE) + xlab("Pattern standard deviation (PSD)") + ylab("Difference in pattern standard deviation (dB)"))
