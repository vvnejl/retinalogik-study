...
mdpsd <- dBdat %>%
  distinct(id, visit, device, eye, md, psd) %>%
  pivot_wider(names_from = device,
              values_from = c(md, psd))

# mdpsd <- dBdat %>%
#   filter(id=="RL12") %>%
#   select(id, visit, device, eye, md, psd) %>%
#   distinct() %>%
#   pivot_wider(id_cols = c(id, visit, eye), # optional vector of columns you do not want affected
#   names_from = device,
#   values_from = c(md, psd) 
# )
